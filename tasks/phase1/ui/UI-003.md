...
1. **Visual timeline** showing analysis stages (Proposition → Pro → Con → Moderator)
...
const STAGES: StageInfo[] = [
  { id: 'proposition', label: 'Proposition', icon: '📋', order: 0 },
  { id: 'pro', label: 'Arguments FOR', icon: '✅', order: 1 },
  { id: 'con', label: 'Arguments AGAINST', icon: '❌', order: 2 },
  { id: 'moderator', label: 'Synthesis', icon: '⚖️', order: 3 },
];
...
- **Design tokens**: Import from `frontend/src/styles/tokens.css` - includes `--color-primary`, `--color-success`, etc.
- **Zustand store**: Use `frontend/src/stores/debate-store.ts` - has `currentPhase`, `turns` state
- **Existing components**: PhaseIndicator in `frontend/src/components/DebateStream/` shows 6-phase progress (can be extended)
...
4. The 6-phase debate structure differs from the 4-stage example in this task - adapt accordingly

### Related Files

- `frontend/src/components/DebateStream/PhaseIndicator.tsx` - Similar progress display
- `frontend/src/stores/debate-store.ts` - Has `currentPhase`, `turns` state to derive progress
- `frontend/src/types/debate.ts` - DebatePhase enum values
