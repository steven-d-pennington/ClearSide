...
- **Design tokens**: `frontend/src/styles/tokens.css` - has colors, spacing, z-index
- **Button component**: `frontend/src/components/ui/Button` - use for hamburger menu close
- **react-router-dom**: Already installed for routing
- **Modal component**: Can be adapted for mobile menu overlay
...
4. **Use existing tokens** - z-index tokens like `--z-index-modal` for sticky header

### Related Files

- `frontend/src/styles/tokens.css` - CSS variables for colors, spacing, z-index
- `frontend/src/components/ui/Button` - Reusable button component
- `frontend/src/components/ui/Modal` - Portal-based overlay (similar to mobile menu)
- `frontend/src/main.tsx` - React Router is already set up

### Test Configuration

```typescript
// Use the custom render from test-utils (includes BrowserRouter)
