    ...
    switch (serviceId) {
      ...
      case 'ffmpeg-stitch': {
        ...
        try {
          ...
          const voiceIds = await reactionLibrary.getAvailableVoices();
          ...
          if (voiceIds.length > 0) {
            ...
            const firstVoice = voiceIds[0] as string;
            const clips = await reactionLibrary.getAllClipsForVoice(firstVoice);
            const existingClips = clips.slice(0, 3);

            for (const clip of existingClips) {
              const clipPath = pathModule.join(REACTIONS_DIR, firstVoice, clip.filename as string);
              try {
                await fs.access(clipPath);
                testFiles.push(clipPath);
              } catch {
          ...
          if (testFiles.length < 2) {
            ...
            for (let i = 0; i < frequencies.length; i++) {
              const tonePath = pathModule.join(tempDir, `test-tone-${i}.mp3`);
          ...
          const outputPath = pathModule.join(tempDir, 'stitched-output.mp3');
          ...
          checks.push({
            name: 'Concatenation',
            success: true,
            message: `Completed in ${concatDuration}ms`,
          });
          ...
          checks.push({
            ...
            message: `File size: ${stats.size} bytes, duration: ${result.durationSeconds?.toFixed(2)}s`,
          });
          ...
          finish(true, 'FFmpeg stitch test passed', {
            ...
            concatDurationMs: concatDuration,
          }, checks);
